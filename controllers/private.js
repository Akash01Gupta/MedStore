...
exports.registerstore = async (req, res, next) => {
    try {
        ...
        // Find the seller based on the userId
        const userExist = await Seller.findById(userId);
        if (!userExist) {
            return res.status(404).json({ success: false, error: "User Not Found" });
        }

        // Ensure userId in the request body matches the one in the request params
        if (userId !== req.params.userid) {
            return res.status(409).json({ success: false, error: "Invalid User" });
        }

        // Combine addressList objects into a single object
        const list = req.body.addressList.reduce((acc, obj) => ({ ...acc, ...obj }), {});

        // Destructure required fields from req.body
        const { storeName, ownerName, ownerEmail, storeAddress, latitude, longitude, ...other } = req.body;
        ...
        // Update the user with the storeId
        await userExist.updateOne({ $set: { storeId: store._id } });
