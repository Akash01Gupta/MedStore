...
export default function Storedashboard(props) {
    ...
    return (
        <div className="storedashboard flex flex-col h-screen">
            {/* Sidebar */}
            <div className={`left ${isOpen ? 'active' : ''} bg-gray-800 text-white`}>
                <ul className="dashboardul">
                    <li id="analytics" className="dashboardListItem cursor-pointer" onClick={changePage}>Analytics</li>
                    <li id="addstoreproduct" className="dashboardListItem cursor-pointer" onClick={changePage} >Add Product</li>
                    <li id="allstoreproduct" className="dashboardListItem cursor-pointer" onClick={changePage}>All Product</li>
                    <li id="orders" className="dashboardListItem cursor-pointer" onClick={changePage}>Orders</li>
                    <li id="setting" className="dashboardListItem cursor-pointer" onClick={changePage}>Setting</li>
                    <hr className="dashHr"></hr>
                    <li id="logout" className="dashListItemLogout cursor-pointer" onClick={logoutHandler}>Logout</li>
                </ul>
            </div>
            {/* Main Content */}
            <div className="right flex-grow p-4">
                { switchComponent() }
            </div>
            {/* Footer */}
